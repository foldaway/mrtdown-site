...
function latestByIssueIdAndType(events: ImpactEventSummary[]) {
  return events.reduce<
    Record<
      string,
      Partial<Record<ImpactEventSummary['type'], ImpactEventSummary>>
    >
  >((acc, event) => {
    acc[event.issue_id] ??= {};
...
export function buildStationIncluded(
  stationRows: Array<
    Pick<
...
async function getLinesForIssueHydrationFromDb(db: AppDb) {
  const [lineRows, lineOperatorRows] = await Promise.all([
    timeDbQuery('issue_q_lines', () =>
      db
        .select({
          id: linesTable.id,
    ...
    timeDbQuery('issue_q_line_operators', () =>
      db
        .select({
...
export async function getIssueData(issueId: string) {
  ...
  return getIssueDataFromDb(db, issueId);
}

export async function getIssueDataFromDb(
  db: AppDb,
  issueId: string,
  referenceNow = nowSg(),
) {
  const [lines, evidenceRows] = await Promise.all([
    getLinesForIssueHydrationFromDb(db),
    timeDbQuery('issue_q_evidence_updates', () =>
      db
        .select({
          type: evidencesTable.type,
          text: evidencesTable.text,
          render: evidencesTable.render,
          source_url: evidencesTable.source_url,
          ts: evidencesTable.ts,
        })
        .from(evidencesTable)
  ...
  const hydration = await getScopedIssueHydrationFromDb({
    db,
    issueIds: [issueId],
    lines,
    referenceNow,
  ...
  const issue = hydration.allIssues[issueId];
  if (issue == null) {
  ...
  return {
    ...
    included: selectIncludedEntities(hydration.included, hydration.allIssues, {
      issueIds: [issueId],
...
export async function getScopedIssueHydrationFromDb({
  db,
  issueIds,
  lines,
  referenceNow,
  ...
  const [issueRows, impactEventRows, latestEvidenceRows] = await Promise.all([
    timeDbQuery(`${spanPrefix}_q_issues`, () =>
    ...
    timeDbQuery(`${spanPrefix}_q_impact_events`, () =>
      selectByIdChunks(selectedIssueIds, (ids) =>
        db
          .select({
    ...
    timeDbQuery(`${spanPrefix}_q_latest_evidence`, () =>
      selectByIdChunks(selectedIssueIds, (ids) =>
        db
          .select({
    ...
    ...new Set(
      Object.values(latestEventByTypeByIssueId).flatMap((latestEventByType) =>
        [
          ...
          .filter((event): event is ImpactEventSummary => event != null)
          .map((event) => event.id),
      ),
    ),
  ];
  ...
  const [
    ...
    timeDbQuery(`${spanPrefix}_q_issue_periods`, () =>
    ...
    timeDbQuery(`${spanPrefix}_q_issue_services`, () =>
    ...
    timeDbQuery(`${spanPrefix}_q_issue_facilities`, () =>
      selectByIdChunks(selectedStateEventIds, (ids) =>
        db
          .select({
    ...
    timeDbQuery(`${spanPrefix}_q_issue_causes`, () =>
      selectByIdChunks(selectedStateEventIds, (ids) =>
        db
          .select({
    ...
    timeDbQuery(`${spanPrefix}_q_issue_service_scopes`, () =>
      selectByIdChunks(selectedStateEventIds, (ids) =>
        db
          .select({
    ...
    timeDbQuery(`${spanPrefix}_q_issue_service_effects`, () =>
      selectByIdChunks(selectedStateEventIds, (ids) =>
        db
          .select({
    ...
    timeDbQuery(`${spanPrefix}_q_issue_facility_effects`, () =>
      selectByIdChunks(selectedStateEventIds, (ids) =>
        db
          .select({
  ...
  const [serviceRows, serviceRevisionRows] = await Promise.all([
    timeDbQuery(`${spanPrefix}_q_services`, () =>
    ...
    timeDbQuery(`${spanPrefix}_q_service_revisions`, () =>
      selectByIdChunks(serviceIds, (ids) =>
        db
          .select({
  ...
  const [stationRows, stationCodeRows] = await Promise.all([
    timeDbQuery(`${spanPrefix}_q_stations`, () =>
    ...
    timeDbQuery(`${spanPrefix}_q_station_codes`, () =>
      selectByIdChunks(allStationIds, (ids) =>
        db
          .select({
