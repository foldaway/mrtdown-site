...
import { buildDataset } from './baseDataset';
...
export async function getOverviewDataFromDb(
  ...
  const needsLegacyIssueHydration =
    candidateIssueIds.length > 0 || communitySignalStationIds.length > 0;
  const issueDataset = needsLegacyIssueHydration
    ? await timeServerSpan('overview_active_issue_hydration', () =>
        buildDataset(referenceNow, db, candidateIssueIds),
      )
    : null;
  const allIssues = issueDataset?.allIssues ?? {};
  ...
  const baseIncluded = issueDataset?.included ?? emptyIncluded(lines);
