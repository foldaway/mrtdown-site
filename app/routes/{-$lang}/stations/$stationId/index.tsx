    ...
    <IncludedEntitiesContext.Provider value={included}>
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-4 pt-1 sm:gap-5 sm:pt-2">
          {(stationProfile.arrivalLines.length > 0 ||
            stationProfile.exits.length > 0) && (
            <StationGuideSection
              arrivalLines={stationProfile.arrivalLines}
              exits={stationProfile.exits}
              isHydrated={isHydrated}
              lineColors={lineColors}
              lineNames={Object.fromEntries(
                Object.entries(included.lines).map(([lineId, line]) => [
                  lineId,
